...
Colorado_PSAT_Data_2022 <-
  fread(file = "Data/Base_Files/SAT_READIN_2022.txt",
        colClasses = rep("character", 26), quote = "'")

Colorado_PSAT_Data_2022[, EDW_DATA_SOURCE := NULL]
Colorado_PSAT_Data_2022[, VENDOR_STUDENT_ID := NULL]

#   Rename YR
setnames(Colorado_PSAT_Data_2022, "YR", "YEAR")

Colorado_PSAT_Data_2022[, CONTENT_AREA :=
...
#   Convert SCALE_SCORE variable to numeric
Colorado_PSAT_Data_2022[, SCALE_SCORE := as.numeric(SCALE_SCORE)]

#   Convert names to factors (temporary to change levels vs values for time/memory saving)
Colorado_PSAT_Data_2022[, DISTRICT_NAME := factor(DISTRICT_NAME)]
Colorado_PSAT_Data_2022[, SCHOOL_NAME := factor(SCHOOL_NAME)]
Colorado_PSAT_Data_2022[, FIRST_NAME := factor(FIRST_NAME)]
Colorado_PSAT_Data_2022[, LAST_NAME := factor(LAST_NAME)]

#   Clean up LAST_NAME and FIRST_NAME
setattr(Colorado_PSAT_Data_2022$LAST_NAME, "levels",
       sapply(levels(Colorado_PSAT_Data_2022$LAST_NAME), SGP::capwords))
setattr(Colorado_PSAT_Data_2022$FIRST_NAME, "levels",
        sapply(levels(Colorado_PSAT_Data_2022$FIRST_NAME), SGP::capwords))

#   Clean up SCHOOL_NAME and DISTRICT_NAME
new.sch.levs <- toupper(levels(Colorado_PSAT_Data_2022$SCHOOL_NAME))
new.sch.levs <- gsub("/", " / ", new.sch.levs)
...
setattr(Colorado_PSAT_Data_2022$SCHOOL_NAME, "levels", new.sch.levs)

###  Districts
grep("J", levels(Colorado_PSAT_Data_2022$DISTRICT_NAME), value = TRUE)
new.dst.levs <- toupper(levels(Colorado_PSAT_Data_2022$DISTRICT_NAME))
new.dst.levs <- gsub("/", " / ", new.dst.levs)
new.dst.levs <- gsub("[-]", " - ", new.dst.levs)
...
setattr(Colorado_PSAT_Data_2022$DISTRICT_NAME, "levels", new.dst.levs)

#  Establish ACHIEVEMENT_LEVEL for PSAT/SAT
Colorado_PSAT_Data_2022 <-
  SGP:::getAchievementLevel(Colorado_PSAT_Data_2022, state = "CO")
Colorado_PSAT_Data_2022[ACHIEVEMENT_LEVEL == "NA",
                            ACHIEVEMENT_LEVEL := "No Score"]
table(Colorado_PSAT_Data_2022[, ACHIEVEMENT_LEVEL, VALID_CASE])

###   Resolve duplicates
# setkey(Colorado_PSAT_Data_2022,
#        VALID_CASE, CONTENT_AREA, YEAR, ID, GRADE, SCALE_SCORE)
# setkey(Colorado_PSAT_Data_2022,
#        VALID_CASE, CONTENT_AREA, YEAR, ID, GRADE)
# dup.index <- which(duplicated(Colorado_PSAT_Data_2022,
#                               by = key(Colorado_PSAT_Data_2022)))
# dups <- data.table(Colorado_PSAT_Data_2022[
#                      unique(c(dup.index - 1, dup.index)), ],
#                    key = key(Colorado_PSAT_Data_2022))
# table(dups$VALID_CASE) # 0 duplicates in 2022
# Colorado_PSAT_Data_2022[dup.index, VALID_CASE := "INVALID_CASE"]

## Save 2022 PSAT_SAT Data
# save(Colorado_PSAT_Data_2022, file="Data/Colorado_PSAT_Data_2022.Rdata")
